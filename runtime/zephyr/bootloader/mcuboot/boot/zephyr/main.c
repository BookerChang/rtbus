...
static void usb_console_setup_line_ctrl(void)
{
    ...
    if (uart_line_ctrl_get(dev, UART_LINE_CTRL_DTR, &dtr) != 0 || dtr == 0) {
        return;
    }
...
#else
static inline void usb_console_setup_line_ctrl(void)
{
}
#endif
...
static void do_boot(struct boot_rsp *rsp)
{
    ...
    (void)udc_disable(DEVICE_DT_GET(DT_NODELABEL(zephyr_udc0)));
#endif
#if CONFIG_MCUBOOT_CLEANUP_ARM_CORE
